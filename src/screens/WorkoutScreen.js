...
function CardioInfoInput({ logExerciseId, initialCardio, onSaved }) {
  const [dist,  setDist]  = useState(initialCardio?.distanceM  ? String(initialCardio.distanceM / 1000) : '');
  const [speed, setSpeed] = useState(initialCardio?.speedKmh   ? String(initialCardio.speedKmh)         : '');
  const [cal,   setCal]   = useState(initialCardio?.calories   ? String(initialCardio.calories)          : '');

  useEffect(() => {
    setDist(initialCardio?.distanceM  ? String(initialCardio.distanceM / 1000) : '');
    setSpeed(initialCardio?.speedKmh  ? String(initialCardio.speedKmh)         : '');
    setCal(initialCardio?.calories    ? String(initialCardio.calories)          : '');
  }, [logExerciseId]);

  const save = async () => {
    await updateCardioInfo(logExerciseId, {
      distanceM: Math.round((parseFloat(dist)  || 0) * 1000), // km → m
  ...
  return (
    <View style={sc.wrap}>
      <Text style={sc.title}>유산소 추가 정보</Text>
      <View style={sc.row}>
        {/* 거리 */}
        <View style={sc.field}>
          <Text style={sc.label}>거리</Text>
          <View style={sc.inputRow}>
            <TextInput
              style={sc.input} keyboardType="decimal-pad"
              value={dist} placeholder="0.0" placeholderTextColor={C.t3}
              onChangeText={setDist} onEndEditing={save} onBlur={save}
            />
            <Text style={sc.unit}>km</Text>
          </View>
        </View>
        {/* 평균 속도 */}
        <View style={sc.field}>
          <Text style={sc.label}>평균속도</Text>
          <View style={sc.inputRow}>
            <TextInput
              style={sc.input} keyboardType="decimal-pad"
              value={speed} placeholder="0.0" placeholderTextColor={C.t3}
              onChangeText={setSpeed} onEndEditing={save} onBlur={save}
            />
            <Text style={sc.unit}>km/h</Text>
          </View>
        </View>
        {/* 칼로리 */}
        <View style={sc.field}>
          <Text style={sc.label}>칼로리</Text>
          <View style={sc.inputRow}>
            <TextInput
              style={sc.input} keyboardType="number-pad"
              value={cal} placeholder="0" placeholderTextColor={C.t3}
              onChangeText={setCal} onEndEditing={save} onBlur={save}
            />
            <Text style={sc.unit}>kcal</Text>
          </View>
        </View>
      </View>
      {/* 요약 뱃지 */}
      {(dist || speed || cal) ? (
        <View style={sc.summary}>
          {dist  ? <View style={sc.badge}><Text style={sc.badgeTxt}>{dist} km</Text></View>  : null}
          {speed ? <View style={sc.badge}><Text style={sc.badgeTxt}>{speed} km/h</Text></View> : null}
          {cal   ? <View style={sc.badge}><Text style={sc.badgeTxt}>{cal} kcal</Text></View>  : null}
        </View>
      ) : null}
    </View>
  );
...
const sc = StyleSheet.create({
  wrap:  { backgroundColor: C.bg2, borderRadius: 8, padding: 10, marginTop: 6, marginHorizontal: 12, marginBottom: 4 },
  title: { fontSize: 10, fontWeight: '600', color: C.t2, letterSpacing: 0.5, textTransform: 'uppercase', marginBottom: 8 },
  row:   { flexDirection: 'row', gap: 8 },
  field: { flex: 1 },
  label: { fontSize: 10, color: C.t2, marginBottom: 3, textAlign: 'center' },
  inputRow:{ flexDirection: 'row', alignItems: 'center', backgroundColor: C.bg1, borderRadius: 6, borderWidth: 1, borderColor: C.border, paddingHorizontal: 6, paddingVertical: 4 },
  input: { flex: 1, fontSize: 14, fontWeight: '500', color: C.t0, textAlign: 'center', padding: 0 },
  unit:  { fontSize: 9, color: C.t2, marginLeft: 2, flexShrink: 0 },
  summary: { flexDirection: 'row', gap: 6, marginTop: 8, flexWrap: 'wrap' },
  badge:   { backgroundColor: C.bg1, borderRadius: 10, paddingHorizontal: 8, paddingVertical: 2, borderWidth: 1, borderColor: C.border },
  badgeTxt:{ fontSize: 11, color: MUSCLE_GROUPS.cardio.color, fontWeight: '500' },
});

// ─── 메모 입력 — 한글 IME 버그 수정 ─────────────────────────────────────────
// onChangeText 마다 DB 저장하면 한글 조합이 끊김
// → 로컬 state로 타이핑, blur/endEditing 시에만 DB 저장

function NotesInput({ date, initialValue }) {
  const [text, setText] = useState(initialValue);

  // 날짜가 바뀌면 초기값 동기화
  useEffect(() => { setText(initialValue); }, [date, initialValue]);

  const save = async (val) => {
    await updateLogNotes(date, val);
  };
  ...
  return (
    <TextInput
      ...
      placeholderTextColor={C.t3}
      value={text}
      onChangeText={t => setText(t)}          // 로컬만 — DB 저장 없음
      onEndEditing={() => save(text)}          // 키보드 완료 시 저장
      onBlur={() => save(text)}               // 포커스 잃을 때 저장
...
const sn = StyleSheet.create({
  notes: {
    fontSize: 13, color: C.t0, padding: 0, marginTop: 4,
...
export default function WorkoutScreen() {
  ...
  // ── 날짜 전환 애니메이션 (fade) ──────────────────────────────────────────
  // slide 방식은 setDate 타이밍과 충돌해 흰 화면 발생 → fade로 교체
  ...
  if (!swipePan.current) {
    swipePan.current = PanResponder.create({
      ...
      onPanResponderRelease: (_, g) => {
        // 왼쪽으로 스와이프(dx<0) → 다음날(delta -1)
        // 오른쪽으로 스와이프(dx>0) → 하루 전(delta +1)
  ...
  const renderEntry = (entry, idx, isDragging, onLongPress) => {
    ...
    return (
      <Card style={[isDragging && { borderColor: C.blue, borderWidth: 2 }]}>
        ...
        {/* 유산소 추가 정보 (접혀있어도 항상 표시) */}
        {ex.group === 'cardio' && (
          <CardioInfoInput
            logExerciseId={entry.id}
            initialCardio={entry.cardio}
            onSaved={reloadLog}
          />
        )}
      </Card>
            ...
            {!log ? (
              ...
              <>
                ...
                <Card style={{ padding: 12 }}>
                  <Label text="메모" />
                  <NotesInput date={date} initialValue={log.notes || ''} />
                </Card>
              </>
            )}
          </ScrollView>
