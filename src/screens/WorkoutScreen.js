...
function Stepper({ value, onChange, step = 1, min = 0, placeholder }) {
  ...
  const adjust = (d) => {
    const next = Math.max(
      min,
      Math.round(((parseFloat(text) || 0) + d) * 100) / 100,
    );
    const s = String(next);
    setText(s);
      ...
      {orderedItems.map((item, idx) => {
        ...
        return (
          <Animated.View
            key={item.id}
            onLayout={(e) => {
              heightsRef.current[item.id] = e.nativeEvent.layout.height;
            }}
            style={[
              { marginBottom: 8 },
              isDragging && {
...
export default function WorkoutScreen() {
  ...
  const animateAndSetDate = useCallback(
    (newDate, direction) => {
  ...
  useEffect(() => {
    if (sharedDate) {
      animateAndSetDate(
        sharedDate,
        sharedDate > currentDateRef.current ? -1 : 1,
      );
      setSharedDate(null);
      setFromCal(true);
    }
  ...
  const handleAddExercise = async (exId) => {
    ...
    if (lastLog?.sets?.length) {
      for (let i = 0; i < lastLog.sets.length; i++)
        await addSet(
          leId,
          lastLog.sets[i].weight,
          lastLog.sets[i].reps,
          i,
          lastLog.sets[i].durationSec || 0,
        );
    } else {
      await addSet(leId, 0, 10, 0, 0);
    }
  ...
  const handleRemoveEntry = (leId) =>
    Alert.alert("운동 삭제", "삭제하시겠습니까?", [
      { text: "취소", style: "cancel" },
      {
        text: "삭제",
        style: "destructive",
        onPress: async () => {
          await removeExerciseFromLog(leId);
          await reloadLog();
        },
      },
    ]);
  ...
  const handleUpdateSet = async (setId, field, value) => {
    ...
    const cur = await db.getFirstAsync("SELECT * FROM sets WHERE id=?", [
      setId,
    ]);
    if (!cur) return;
    await updateSet(
  ...
  const handleReorder = useCallback(async (newOrderIds) => {
    ...
    setLog((prev) => {
      ...
      return {
        ...prev,
        exercises: newOrderIds.map((id) => map[id]).filter(Boolean),
      };
    });
  }, []);
        ...
        >
          ...
          {isDragging && (
            <View style={s.movingBadge}>
              <Text style={s.movingText}>이동 중</Text>
            </View>
          )}
          {!isDragging &&
          ...
          {isDur && (
            <View style={s.typeBadge}>
              <Text style={s.typeBadgeText}>시간</Text>
            </View>
          )}
        ...
        <View style={s.badgeRow}>
          {entryVol > 0 && (
            <View style={s.volPill}>
              <Text style={s.volPillText}>
                {isDur
                  ? `${entryVol.toFixed(0)}분`
                  : `${entryVol.toLocaleString()} kg`}
              </Text>
            </View>
          )}
          {meta.lastVol > 0 && (
            <View style={s.prevPill}>
              <Text style={s.prevPillText}>
                직전{" "}
                {isDur
                  ? `${meta.lastVol.toFixed(0)}분`
                  : `${meta.lastVol.toLocaleString()} kg`}
              </Text>
            </View>
          )}
          {!isDur && meta.prW > 0 && (
            <View style={s.prPill}>
              <Text style={s.prPillText}>PR {meta.prW} kg</Text>
            </View>
          )}
        </View>
          ...
          >
            {isDur ? (
              <View style={[s.setRow, { paddingVertical: 4 }]}>
                <View style={{ width: 18 }} />
                <Text style={s.colHeader}>시간 (분)</Text>
                <View style={{ width: 28 }} />
                <View style={{ width: 28 }} />
              </View>
            ) : (
            ...
            {entry.sets.map((set, si) => {
              ...
              const isPR =
                ...
                ls &&
                set.done &&
                (set.weight > ls.weight ||
                  (set.weight === ls.weight && set.reps > ls.reps));
              return (
                <View key={set.id || si} style={s.setRow}>
                  ...
                  {isDur ? (
                    <View style={{ flex: 2 }}>
                      <Stepper
                        ...
                        placeholder={
                          ls?.durationSec
                            ? String(Math.round(ls.durationSec / 60))
                            : "0"
                        }
                        onChange={(v) =>
                          handleUpdateSet(
                      ...
                      {ls?.durationSec > 0 && (
                        <Text style={s.prevHint}>
                          직전 {Math.round(ls.durationSec / 60)}분
                        </Text>
                      )}
                    </View>
                  ) : (
                    <>
                      ...
                      <View style={{ flex: 1 }}>
                        <Stepper
                          ...
                          onChange={(v) =>
                            handleUpdateSet(set.id, "reps", Math.round(v))
                          }
                        />
                  ...
                  >
                    <Text
                      style={{ color: set.done ? "#fff" : C.t2, fontSize: 12 }}
                    >
                      ✓
                    </Text>
                  </TouchableOpacity>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      ...
      <View
        style={{ flex: 1 }}
        {...(scrollEnabled ? swipePan.current.panHandlers : {})}
      >
        <Animated.View
          ...
          >
            ...
            <View style={s.dateNav}>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  animateAndSetDate(todayStr(), 0);
                  setFromCal(false);
                }}
              >
                <Text style={[s.todayBtnTxt, isToday && { color: "#fff" }]}>
                  오늘
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
            ...
            {!loading && !log && (
              <Text style={s.swipeHint}>← 스와이프로 날짜 이동 →</Text>
            )}
            ...
            {loading ? (
              ...
              <>
                ...
                {log.exercises.length === 0 && (
                  <Text style={s.hintText}>운동을 추가해주세요</Text>
                )}
                {log.exercises.length > 1 && (
                  <Text style={s.dragHint}>길게 눌러서 순서 변경</Text>
                )}
              ...
              >
                ...
                >
                  <View
                    style={[
                      s.typeBadgeSm,
                      ex.inputType === "duration" && s.typeDurSm,
                    ]}
                  >
                    <Text style={s.typeSmTxt}>
                      {ex.inputType === "duration" ? "시간" : "중량"}
                    </Text>
                  </View>
                  <Chip group={ex.group} />
                  {already && (
                    <Text style={{ fontSize: 10, color: C.t2 }}>추가됨</Text>
                  )}
                </View>
              </TouchableOpacity>
            );
