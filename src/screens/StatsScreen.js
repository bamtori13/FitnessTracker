  ...
  return (
    <View>
      <View style={{ flexDirection: 'row' }}>
        {/* Y축 */}
        <View style={{ width: AXIS_L, height: height, justifyContent: 'space-between', alignItems: 'flex-end', paddingRight: 4 }}>
          {[...yTicks].reverse().map((v, i) => (
            <Text key={i} style={gs.axisLabel}>{fmtVol(v)}</Text>
          ))}
        </View>
  ...
  return (
    <View>
      ...
      <View style={{ flexDirection: 'row', marginLeft: AXIS_L, marginTop: 3 }}>
        {points.map((p, i) => (
          <View key={i} style={{ position: 'absolute', left: xPos(i) - 10, width: 20, alignItems: 'center' }}>
...
function ExBarChart({ data, color, height = CHART_H }) {
  ...
  return (
    <View>
      <View style={{ flexDirection: 'row' }}>
        {/* Y축 */}
        <View style={{ width: AXIS_L, height, justifyContent: 'space-between', alignItems: 'flex-end', paddingRight: 4 }}>
          {[...yTicks].reverse().map((v, i) => (
            <Text key={i} style={gs.axisLabel}>{fmtVol(v)}</Text>
          ))}
        </View>
        ...
        {/* 바 영역 */}
        <View style={{ flex: 1, height, position: 'relative', flexDirection: 'row', alignItems: 'flex-end', gap: 3 }}>
          {yTicks.map((v, i) => (
            <View key={i} style={{
              position: 'absolute', left: 0, right: 0,
              top: yPos(v), height: 1,
          ...
          {data.map((d, i) => {
            const barH = d.vol ? Math.max(4, Math.round((d.vol / yMax) * height)) : 4;
            return (
              <View key={i} style={{ flex: 1 }}>
                <View style={{ height: barH, backgroundColor: d.vol ? color : C.bg3, borderRadius: 3 }} />
              </View>
            );
          })}
        </View>
...
export default function StatsScreen() {
  ...
  useEffect(() => {
    if (tab !== 0) return;
    setLoading(true);
    Promise.all([getOverallStats(), getGroupVolumeLast14Days()]).then(([ov, rows]) => {
      setOverview(ov);
      ...
      const series = Object.entries(MUSCLE_GROUPS).map(([grp, gv]) => {
  ...
  useEffect(() => {
    ...
    getExerciseSeriesByGroup(selGroup, ids, 10).then(series => {
      ...
      const result = series.map((s, i) => {
        ...
        const alpha = Math.round(180 + 70 * (i / Math.max(series.length - 1, 1)));
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      ...
      <ScrollView contentContainerStyle={gs.page}>
        ...
        {/* ── 개요 ─────────────────────────────────────── */}
        {tab === 0 && (loading || !overview ? <Loading /> : (
          <>
            <View style={gs.statGrid}>
              <StatBox label="총 운동일" value={overview.totalDays} unit="일" color={C.blue} style={{ flex: 1 }} />
              <StatBox label="최근 30일" value={overview.last30} unit="회" style={{ flex: 1 }} />
            </View>
            ...
            <Card style={{ padding: 12 }}>
              ...
              {lineSeries.length === 0 ? (
                ...
                <>
                  ...
                  <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 10, marginLeft: AXIS_L }}>
                    {lineSeries.map(s => (
                      <View key={s.group} style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                        <View style={{ width: 18, height: 3, borderRadius: 2, backgroundColor: s.color }} />
                        <Text style={{ fontSize: 10, color: C.t2 }}>{MUSCLE_GROUPS[s.group]?.label}</Text>
                      </View>
                    ))}
                  </View>
                </>
              ...
              <View style={{ flexDirection: 'row', gap: 5 }}>
                {Object.entries(MUSCLE_GROUPS).map(([k, v]) => (
                  <TouchableOpacity key={k} onPress={() => setSelGroup(k)}
                    ...
                    <Text style={[gs.groupBtnText, selGroup === k && { color: v.color }]}>{v.label}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </ScrollView>

            {loading ? <Loading /> : exSeriesData.length === 0 ? (
              ...
              const opacity = 0.55 + 0.45 * (si / Math.max(exSeriesData.length - 1, 1));
              const isDur   = exercises.find(e => e.id === ser.exId)?.inputType === 'duration';
                ...
                <Card key={ser.exId} style={{ padding: 12, marginBottom: 10 }}>
                  ...
                  <View style={{ flexDirection:'row', alignItems:'center', justifyContent:'space-between', marginBottom:8 }}>
                    ...
                    <View style={[gs.sessionBadge, { backgroundColor: MUSCLE_GROUPS[selGroup]?.bg }]}>
                      <Text style={[gs.sessionBadgeText, { color: gColor }]}>최근 {n}회</Text>
                    </View>
                  </View>
                  ...
                  <View style={{ flexDirection: 'row' }}>
                    {/* Y축 */}
                    <View style={{ width:AXIS_L, height:CHART_H, justifyContent:'space-between', alignItems:'flex-end', paddingRight:5 }}>
                      {[...yTicks].reverse().map((v, i) => (
                        <Text key={i} style={gs.axisLabel}>{fmtVol(v)}</Text>
                      ))}
                    </View>
                    ...
                    <View style={{ flex:1, height:CHART_H, position:'relative' }}>
                      ...
                      {segs.map((seg, i) => (
                        <View key={i} style={{
                          ...
                          width:seg.len, height:2.5,
                          backgroundColor:gColor, opacity,
                          transform:[{ rotate:`${seg.ang}deg` }],
                          transformOrigin:'left center',
                        }} />
                      ))}
                  ...
                  <View style={{ flexDirection:'row', marginLeft:AXIS_L, marginTop:3 }}>
                    {pts.map((_, i) => (
                      <View key={i} style={{ flex: n > 1 ? 1 : 0, width: n === 1 ? chartW : undefined, alignItems:'center' }}>
                  ...
                  {n >= 2 && (() => {
                    ...
                    return (
                      <View style={gs.seriesSummary}>
                        <Text style={gs.seriesSumText}>최고 {fmtVol(maxV)}{yUnit}</Text>
                        <Text style={[gs.seriesSumText, { color: tc, fontWeight:'600' }]}>
                          {trend} {Math.abs(diff).toFixed(0)}{yUnit}
                        </Text>
                        <Text style={gs.seriesSumText}>마지막 {fmtVol(vols[n-1])}{yUnit}</Text>
                      </View>
                    );
                  })()}
                </Card>
        ...
        {tab === 2 && (
          <>
            <View style={gs.pickerWrap}>
              <RNPicker
                selectedValue={selEx}
                onValueChange={v => { setSelEx(v); setExData([]); setExMaxW(0); }}
                style={gs.picker}
              >
                <RNPicker.Item label="종목 선택..." value="" color={C.t2} />
                {Object.entries(MUSCLE_GROUPS).map(([gk, gv]) =>
                  exercises.filter(e => e.group === gk).map(ex => (
                    <RNPicker.Item
                      key={ex.id}
            ...
            {selEx && (loading ? <Loading /> : (
              <>
                <View style={gs.statGrid}>
                  <StatBox label="최대 중량" value={`${exMaxW}kg`} style={{ flex: 1 }} />
                  <StatBox label="총 세션"   value={`${exData.length}회`} style={{ flex: 1 }} />
                </View>
                ...
                <Card style={{ padding: 12, marginTop: 10 }}>
                  ...
                  {exData.length === 0
                    ...
                    : <ExBarChart
                        data={exData}
                        color={MUSCLE_GROUPS[exercises.find(e => e.id === selEx)?.group]?.color || C.blue}
                      />
                  }
                </Card>
                ...
                <Card style={{ padding: 12, marginTop: 10 }}>
                  ...
                  <View style={gs.histHeader}>
                    <Text style={[gs.histCell, { flex: 1.2, color: C.t2 }]}>날짜</Text>
                    <Text style={[gs.histCell, { color: C.t2 }]}>최대 중량</Text>
                    <Text style={[gs.histCell, { color: C.t2 }]}>총 볼륨</Text>
                  </View>
                  {exData.slice().reverse().map((d, i) => (
                    <View key={i} style={gs.histRow}>
                      <Text style={[gs.histCell, { flex: 1.2, color: C.t2 }]}>{d.date}</Text>
                      <Text style={[gs.histCell, { fontWeight: '500' }]}>{d.maxW}kg</Text>
                      <Text style={[gs.histCell, { color: C.t1 }]}>{fmtVol(d.vol)}kg</Text>
                    </View>
                  ))}
                </Card>
              </>
...
const gs = StyleSheet.create({
  tabBar: {
    flexDirection: 'row', gap: 5, padding: 12,
    backgroundColor: C.bg1, borderBottomWidth: 1, borderBottomColor: C.border,
  },
  tabBtn:    { flex: 1, paddingVertical: 8, borderRadius: 8, borderWidth: 1, borderColor: C.border, backgroundColor: C.bg1, alignItems: 'center' },
  ...
  inlineBox:   { backgroundColor: C.bg2, borderRadius: 8, padding: 12, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  ...
  groupBtn:     { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 14, borderWidth: 1, borderColor: C.border, backgroundColor: C.bg1 },
  groupBtnText: { fontSize: 11, color: C.t1 },

  pickerWrap: { backgroundColor: C.bg1, borderRadius: 10, borderWidth: 1, borderColor: C.border, overflow: 'hidden', marginBottom: 12 },
  picker:     { color: C.t0 },

  histHeader: { flexDirection: 'row', paddingVertical: 4, borderBottomWidth: 1, borderBottomColor: C.bg3 },
  histRow:    { flexDirection: 'row', paddingVertical: 6, borderBottomWidth: 1, borderBottomColor: C.bg2 },
  ...
  seriesSummary: {
    ...
    borderTopWidth: 1, borderTopColor: C.bg2,
  },
